...
interface IS {
  fetchListOfPeopleRequest: RequestStateProperty<
...
const initialState: IS = {
  fetchListOfPeopleRequest: makeRequestStateProperty<
    ListResponse<ListOfPeopleItem>,
    unknown
  >(),
...
const slice = createSlice({
  ...
  extraReducers: (builder) => {
    makeRequestExtraReducer<RequestList<IS>>(
      builder,
      fetchListOfPeopleThunk,
      'fetchListOfPeopleRequest',
    );
    makeRequestExtraReducer<RequestList<IS>>(
      builder,
      fetchPeopleItemThunk,
      'fetchPeopleItemRequest',
    );
  },
});
...
const fetchPeopleItemThunk = createAsyncThunk(
  `${SLICE_NAME}/fetchPeopleItemThunk`,
